...
#' # 
#' # 
#' # xgx_stat_smooth_ordinal <- function(mapping = NULL, data = NULL,
#' #                                 geom = "ribbon",
#' #                                 position = "identity",
#' #                                 ...,
#' #                                 formula = "y ~ x",
...
#' #       n_bootstrap = n_bootstrap,
#' #       n_resolution = n_resolution,
#' #       y_as_probability = y_as_probability,
#' #       ...
#' #     )
#' #   )
#' # }
#' # StatOrdinalModel <- ggproto("StatOrdinalModel", Stat, 
#' #       required_aes = c("x", "y"),
#' #       compute_group = function(data, scales, formula, n_bootstrap, n_resolution, y_as_probability) {
#' #         return(data)
#' #       },
#' # 
#' #       setup_data = function(data, params) {
#' #         data <- xgx_ordinal_bootstrap_predictions(data = data,
#' #                                                      formula = params$formula,
#' #                                                      n_bootstrap = params$n_bootstrap,
#' #                                                      n_resolution = params$n_resolution,
#' #                                                      y_as_probability = params$y_as_probability)
#' # 
#' # 
#' #         # Get the number of each category in each bin 
#' #         counts <- data %>% group_by(x, colour, PANEL, group) %>%
...
#' #' #' @usage NULL
#' #' #' @export
#' #' StatSmoothOrdinal <- ggproto("StatSmooth", Stat,
#' #'                       setup_params = function(data, params) {
#' #'                         params$flipped_aes <- has_flipped_aes(data, params, ambiguous = TRUE)
#' #'                         msg <- character()
...
#' #'                       extra_params = c("na.rm", "orientation"),
#' #'                       
#' #'                       setup_data = function(data, scales, method = NULL, formula = NULL,
#' #'                                                se = TRUE, n = 80, span = 0.75, fullrange = FALSE,
#' #'                                                xseq = NULL, level = 0.95, method.args = list(),
...
#' #' 
#' #' #' @export
#' #' predictdf.ordinal <- function(model, xseq, se, level) {
#' #'   pred_df <- xgx_ordinal_bootstrap_predictions(data = data,
#' #'                                                formula = formula,
#' #'                                                n_bootstrap = n_bootstrap,
#' #'                                                n_resolution = n_resolution,
#' #'                                                y_as_probability = TRUE)
#' #'   return(data.frame(x = pred_df$x,
#' #'                     y = pred_df$y,
#' #'                     ymin = quantile(pred_df$y)[1],
#' #'                     ymax = quantile(pred_df$y)[2]))
#' #'   pred_df
#' #'   # pred <- MASS::polr(model, newdata = data_frame(x = xseq), se.fit = se,
#' #'   #                        type = "link")
#' #'   # 
#' #'   # if (se) {
...
#' #' #' @param n_bootstrap Number of iterations used in bootstrapping confidence intervals
#' #' xgx_geom_smooth_oridinal <- function(mapping = NULL,
#' #'                                      data = NULL,
#' #'                                      geom = "ribbon",
#' #'                                      position = "identity",
#' #'                                      na.rm = FALSE,
...
#' #'     show.legend = show.legend,
#' #'     inherit.aes = inherit.aes,
#' #'     params = list(n_bootstrap = n_bootstrap,
#' #'                   n_resolution = n_resolution,
#' #'                   na.rm = na.rm,
#' #'                   ...)
#' #'   )
...
#' #       }
#' #     }
#' #     data <- data %>% mutate(Response = factor(data$y,
#' #                                               levels = levels,
#' #                                               ordered = TRUE))
#' #     # Bootstrap for CIs
#' #     pred_df <- NULL
...
#' #     # Convert Probabilities to Classes
#' #     names(pred_df) <- c("x", levels)
#' #     pred_df$pred <- levels[apply(pred_df[levels],1,which.max)]
#' #     pred_df <- pivot_longer(data = pred_df, cols = levels)
#' #     return(pred_df)
#' #   }
#' # )
