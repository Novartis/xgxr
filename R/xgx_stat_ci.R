                  ...
                  compute_group = function(data, scales, conf_level, distribution, bins) {
                    stat_data <- data %>% mutate(quantile_index = dplyr::ntile(data$x, bins)) %>%
                      ...
                      summarize(x = median(x),
                                ymin = conf_int(y, conf_level, distribution)$ymin,
                                ymax = conf_int(y, conf_level, distribution)$ymax,
                                y = conf_int(y, conf_level, distribution)$y) %>%
                      ungroup()
                    return(stat_data)
                  }
)
...
xgx_stat_ci <- function(mapping = NULL,
  ...
  for (igeom in geom) {
    if (!is.null(bins)){
      ...
      # temp <- ggplot2::stat_summary_bin(data = data, inherit.aes = TRUE)
      temp <- ggplot2::stat_summary_bin(mapping = mapping,
                                        data = data,
                                        geom = igeom,
                                        position = position,
                                        ...,
                                        ...
                                        inherit.aes = inherit.aes,
                                        fun.data = function(y) conf_int(y, conf_level, distribution)
                                        # breaks = quantile(self$aes_params$x, seq(0,1,1/bins)) #eval(function(x) quantile(x, seq(0,1, 1/bins)))
                                        
                                        ## Binwidth and bins are overridden
                                        ## to enforce spacing by quantile
                                        #, binwidth = NULL, bins = 30
                                        
      )
      
      # Add breaks here - now that `x` is defined by inhereting aes
      # temp$breaks <- quantile(temp$aes_params$x, seq(0,1,1/bins))
    }
    else{
      temp <- ggplot2::stat_summary(mapping = mapping,
                                    data = data,
                                    geom = igeom,
                                    position = position,
                                    ...,
