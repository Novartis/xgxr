...
xgx_scale_x_time_units <- function(units_dataset, units_plot = NULL,
  ...
  if (missing(units_dataset)) {
    if (requireNamespace("ggforce", quietly = TRUE)) {
        ret <- list(ggforce::scale_x_unit(breaks = breaks,
                                          labels = labels,
                                          ## Hack to capture
                                          ## unit information by
                                          ## transforming then saving
                                          ## the info in the package
                                          trans = xgx_1_unit(),
                                          ...))
    } else {
      stop("package 'ggforce' needs to be installed for plotting units")
    }
  } else {
    ret <- list(
