...
([outlined here](http://opensource.nibr.com/xgx)).  It also contains helper 
...
g <- xgx_plot(data = data, aes(x = x, y = y)) +
  geom_point()
xgx_save(width = 4, height = 4, "Example_Plot", "DRAFT", dirs = dirs)
```

The the function `xgx_save` works only with ggplot objets.  If the figure 
...
xgx_annotate_status_png("png_example.png", "./ExampleScript.R")
```
...
data <- xgx_save_table(x, "ExampleTable", dirs)
knitr::kable(data)
```
...
allows the definition of multple `geom` options in one function call, defined 
