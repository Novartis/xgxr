...
```{r, error = TRUE, warning=FALSE, message=FALSE}
#load dataset
pkpd_data <- read.csv("https://raw.githubusercontent.com/Novartis/xgxr/master/data_create/raw/nonlinear_pkpd.csv")

DOSE_CMT = 1
PK_CMT = 5
PD_CMT = 4
SS_PROFDAY = 0 # steady state prof day
PD_PROFDAYS <- c(0, 2, 4, 6)
TAU = 244 # time between doses, units should match units of TIME, e.g. 24 for QD, 12 for BID, 7*24 for Q1W (when units of TIME are h)

#ensure dataset has all the necessary columns
pkpd_data = pkpd_data %>%
  mutate(TIME = TIM2, NOMTIME = NT, EVID = 0, CENS = 0, DOSE = MGKG,     TRTACT = TRT, LIDV_NORM = LIDV/MGKG, LIDV_UNIT = UNIT,     PROFDAY = 1, SEX = 0, WEIGHTB = 0, ID = ID, LIDV = LIDV,     CMT = CMT, DAY_label = ifelse(PROFDAY > 0, paste("Day", PROFDAY), "Baseline"))
